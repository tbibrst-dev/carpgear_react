...
/** @var string $id */
/** @var \FS\PickupPointsPro\Api\Model\Point|null $pickup_point */
/** @var bool $disabled */
...
<div class="dpd_uk-package">
	...
	try {
		...
		$args  = [
			...
			'custom_attributes' => [
				'data-ship_to_shop_services' => esc_attr( json_encode( $dpd_uk->get_api()->get_api_data()->get_ship_to_shop_services(), JSON_HEX_APOS ) ),
			],
			'class'             => [ 'dpd_uk-service' ],
			'input_class'       => [ 'dpd_uk-service' ],
		];
		$value = '';
	...
	<?php do_action( 'flexible-shipping-pickup-points/integration/point-selection', 'dpd_uk', $id, $pickup_point, $order->get_shipping_country(), 'dpd', $disabled ); ?>

	<p class="pickup-points-not-available dpd_uk-hidden">
		<?php echo wp_kses_post( sprintf(
			__( 'Sending the shipments via DPD UK Ship to Shop service requires the %1$sDPD UK Pickup Points WooCommerce plugin%2$s to be installed and active as well.', 'woocommerce-dpd-uk' ),
			'<a href="https://octolize.com/product/dpd-uk-dpd-local-pickup-points-woocommerce/?utm_source=dpd-uk&utm_medium=dpd-uk-box&utm_campaign=dpduk-pp" target="_blank">',
			'</a>'
		) ); ?>
	</p>

...
use FS\PickupPointsPro\Api\Model\Point;
...
class WPDesk_Flexible_Shipping_Shipment_dpd_uk extends WPDesk_Flexible_Shipping_Shipment implements WPDesk_Flexible_Shipping_Shipment_Interface {
	...
	const DELIVERY_POINT_NAME     = '_delivery_point_name';
	const DELIVERY_POINT_LAT      = '_delivery_point_lat';
	const DELIVERY_POINT_LONG     = '_delivery_point_long';
	const DELIVERY_POINT_ADDRESS  = '_delivery_point_address';
	const DELIVERY_POINT_POSTCODE = '_delivery_point_postcode';
	const DELIVERY_POINT_CITY     = '_delivery_point_city';
	const DELIVERY_POINT_COUNTRY  = '_delivery_point_country';
	const DPD_UK_SERVICE          = '_dpd_uk_service';
	...
	public function checkout( array $fs_method, $package ) {
		...
		$this->set_meta( '_fs_method', $fs_method );

		$this->set_meta( self::DPD_UK_SERVICE, $fs_method['dpd_uk_service'] );
		...
		$this->set_pickup_point_data_if_present();
	}

	private function set_pickup_point_data_if_present() {
		$pickup_point = apply_filters(
			'flexible-shipping-pickup-points/order/shipment/' . $this->get_meta( '_shipping_id' ) . '/point_details',
			false
		);
		if ( $pickup_point instanceof FS\PickupPointsPro\Api\Model\Point ) {
			$this->set_meta( self::DELIVERY_POINT_LAT, $pickup_point->get_lat() );
			$this->set_meta( self::DELIVERY_POINT_LONG, $pickup_point->get_long() );
			$this->set_meta( self::DELIVERY_POINT_NAME, $pickup_point->get_point_name() );
			$this->set_meta( self::DELIVERY_POINT_CITY, $pickup_point->get_city() );
			$this->set_meta( self::DELIVERY_POINT_POSTCODE, $pickup_point->get_post_code() );
			$this->set_meta( self::DELIVERY_POINT_ADDRESS, $pickup_point->get_address() );
			$this->set_meta( self::DELIVERY_POINT_COUNTRY, $pickup_point->get_country() );
		}
	}
	...
	public function order_metabox_content() {
		...
		$dpd_uk_service = $this->get_meta( self::DPD_UK_SERVICE );
		...
		$pickup_point_name = $this->get_meta( self::DELIVERY_POINT_NAME, null );

		if ( class_exists( Point::class ) ) {
			if ( $pickup_point_name ) {
				$point_data   = [
					'lat'      => $this->get_meta( self::DELIVERY_POINT_LAT, '' ),
					'long'     => $this->get_meta( self::DELIVERY_POINT_LONG, '' ),
					'name'     => $this->get_meta( self::DELIVERY_POINT_NAME, '' ),
					'address'  => $this->get_meta( self::DELIVERY_POINT_ADDRESS, '' ),
					'postCode' => $this->get_meta( self::DELIVERY_POINT_POSTCODE, '' ),
					'city'     => $this->get_meta( self::DELIVERY_POINT_CITY, '' ),
					'country'  => $this->get_meta( self::DELIVERY_POINT_COUNTRY, '' ),
				];
				$pickup_point = new Point( $point_data );
			} else {
				$pickup_point = new Point( [] );
			}
		} else {
			$pickup_point = null;
		}
	...
	public function save_ajax_data( $data ) {
		...
		$this->set_or_remove( $data, self::DELIVERY_POINT_LAT );
		$this->set_or_remove( $data, self::DELIVERY_POINT_LONG );
		$this->set_or_remove( $data, self::DELIVERY_POINT_NAME );
		$this->set_or_remove( $data, self::DELIVERY_POINT_CITY );
		$this->set_or_remove( $data, self::DELIVERY_POINT_POSTCODE );
		$this->set_or_remove( $data, self::DELIVERY_POINT_ADDRESS );
		$this->set_or_remove( $data, self::DELIVERY_POINT_COUNTRY );
	}

	private function save_point_data( $data ) {
		...
		if ( ! $this->label_avaliable() ) {
			...
			throw new Exception(
				...
				// Translators: order number.
					esc_html__( 'The requested operation cannot be performed for the order #%1$d. Please create the shipment first and try again.', 'woocommerce-dpd-uk' ),
					(int) $order->get_id()
				)
			);
		}
	...
	public function get_email_after_order_table() {
		if ( $this->label_avaliable() && $this->get_shipping_method()->get_option( 'email_tracking_number' ) === 'yes' ) {
			$args = array(

...
jQuery(document).on('change','.dpd_uk-service', function() {
	let ship_to_shop_services = jQuery( this ).data( 'ship_to_shop_services' );
	if ( ship_to_shop_services === undefined ) {
		return;
	}
	let selected = jQuery( this ).val();
	let is_ship_to_shop = false;
	ship_to_shop_services.forEach( function ( element ) {
		is_ship_to_shop = is_ship_to_shop || element === selected;
	} );
	if ( jQuery( this ).closest( '.dpd_uk-package' ).find( '.pickup-point' ).length ) {
		jQuery( this ).closest( '.dpd_uk-package' ).find( '.pickup-point' ).toggle( is_ship_to_shop );
		jQuery( this ).closest( '.dpd_uk-package' ).find( '.pickup-point-data' ).toggle( is_ship_to_shop );
	} else {
		jQuery( this ).closest( '.dpd_uk-package' ).find( '.pickup-points-not-available' ).toggle( is_ship_to_shop );
	}
});

function dpd_uk_init() {
	jQuery('.dpd_uk-service').each(function(){
		jQuery(this).change();
	})

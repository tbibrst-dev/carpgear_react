 ...
 */
class Onboarding implements \DpdUKVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const VERSION = 2;
     ...
     */
    public function __construct(string $onboarding_id, bool $autostart, \DpdUKVendor\Octolize\Onboarding\OnboardingShouldShowStrategy $onboarding_should_show_strategy, array $steps, \DpdUKVendor\Octolize\Onboarding\OnboardingAjax $tracker_ajax, \DpdUKVendor\Octolize\Onboarding\OnboardingOption $onboarding_option)
    {
    ...
    {
        if ($this->autostart) {
            $this->onboarding_option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::AUTO_SHOW_POPUP, 1);
            $this->onboarding_option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::VIEWS, (int) $this->onboarding_option->get_option_value(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::VIEWS, 0) + 1);
        }
        $this->onboarding_option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::PAGE, \sanitize_text_field($page));
        $this->onboarding_option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT_TIME, \time());
        $this->onboarding_option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT, 'render');
    }
    /**
     * @return bool
     */

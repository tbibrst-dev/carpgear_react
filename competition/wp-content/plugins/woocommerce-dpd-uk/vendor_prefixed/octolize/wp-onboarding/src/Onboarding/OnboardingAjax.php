 ...
 */
class OnboardingAjax implements \DpdUKVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(\DpdUKVendor\Octolize\Onboarding\OnboardingOption $option)
    {
        $this->option = $option;
    }
    /**
    ...
    public function handle_ajax_action_event()
    {
        ...
        if ($event) {
            $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT, \sanitize_text_field($event));
            $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::STEP, \sanitize_text_field($step));
            $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT_TIME, \time());
    ...
    public function handle_ajax_action_auto_show_popup()
    {
        ...
        $step = $this->filter_input(\INPUT_POST, 'step');
        $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::PAGE, \sanitize_text_field($page));
        $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::STEP, \sanitize_text_field($step));
        $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT_TIME, \time());
        $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::EVENT, 'auto_show');
        \wp_send_json_success();
    }
    public function handle_ajax_action_save_fields()
    {
        ...
        $this->option->update_option(\DpdUKVendor\Octolize\Onboarding\OnboardingOption::SAVE_FIELDS, \sanitize_text_field($data['popup_id']));

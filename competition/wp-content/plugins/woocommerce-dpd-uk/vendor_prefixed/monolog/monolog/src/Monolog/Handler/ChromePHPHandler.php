...
class ChromePHPHandler extends \DpdUKVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function send() : void
    {
        ...
        $json = \DpdUKVendor\Monolog\Utils::jsonEncode(self::$json, \DpdUKVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \DpdUKVendor\Monolog\Logger::WARNING, 'level_name' => \DpdUKVendor\Monolog\Logger::getLevelName(\DpdUKVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];

...
class BrowserConsoleHandler extends \DpdUKVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return [\DpdUKVendor\Monolog\Logger::DEBUG => 'debug', \DpdUKVendor\Monolog\Logger::INFO => 'info', \DpdUKVendor\Monolog\Logger::NOTICE => 'info', \DpdUKVendor\Monolog\Logger::WARNING => 'warn', \DpdUKVendor\Monolog\Logger::ERROR => 'error', \DpdUKVendor\Monolog\Logger::CRITICAL => 'error', \DpdUKVendor\Monolog\Logger::ALERT => 'error', \DpdUKVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \DpdUKVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \DpdUKVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \DpdUKVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**

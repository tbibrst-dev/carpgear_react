...
class ErrorHandler
{
    ...
    /** @var LogLevel::* */
    private $fatalLevel = \DpdUKVendor\Psr\Log\LogLevel::ALERT;
    /** @var ?string */
    ...
    public function __construct(\DpdUKVendor\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
     ...
     * @return ErrorHandler
     */
    public static function register(\DpdUKVendor\Psr\Log\LoggerInterface $logger, $errorLevelMap = [], $exceptionLevelMap = [], $fatalLevel = null) : self
    {
    ...
    public function registerFatalHandler($level = null, int $reservedMemorySize = 20) : self
    {
        ...
        $this->fatalLevel = null === $level ? \DpdUKVendor\Psr\Log\LogLevel::ALERT : $level;
    ...
    {
        return ['ParseError' => \DpdUKVendor\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \DpdUKVendor\Psr\Log\LogLevel::ERROR];
    }
    /**
    ...
    {
        return [\E_ERROR => \DpdUKVendor\Psr\Log\LogLevel::CRITICAL, \E_WARNING => \DpdUKVendor\Psr\Log\LogLevel::WARNING, \E_PARSE => \DpdUKVendor\Psr\Log\LogLevel::ALERT, \E_NOTICE => \DpdUKVendor\Psr\Log\LogLevel::NOTICE, \E_CORE_ERROR => \DpdUKVendor\Psr\Log\LogLevel::CRITICAL, \E_CORE_WARNING => \DpdUKVendor\Psr\Log\LogLevel::WARNING, \E_COMPILE_ERROR => \DpdUKVendor\Psr\Log\LogLevel::ALERT, \E_COMPILE_WARNING => \DpdUKVendor\Psr\Log\LogLevel::WARNING, \E_USER_ERROR => \DpdUKVendor\Psr\Log\LogLevel::ERROR, \E_USER_WARNING => \DpdUKVendor\Psr\Log\LogLevel::WARNING, \E_USER_NOTICE => \DpdUKVendor\Psr\Log\LogLevel::NOTICE, \E_STRICT => \DpdUKVendor\Psr\Log\LogLevel::NOTICE, \E_RECOVERABLE_ERROR => \DpdUKVendor\Psr\Log\LogLevel::ERROR, \E_DEPRECATED => \DpdUKVendor\Psr\Log\LogLevel::NOTICE, \E_USER_DEPRECATED => \DpdUKVendor\Psr\Log\LogLevel::NOTICE];
    }
    /**
    ...
    {
        $level = \DpdUKVendor\Psr\Log\LogLevel::ERROR;
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \DpdUKVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError(int $code, string $message, string $file = '', int $line = 0, ?array $context = []) : bool
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \DpdUKVendor\Psr\Log\LogLevel::CRITICAL;

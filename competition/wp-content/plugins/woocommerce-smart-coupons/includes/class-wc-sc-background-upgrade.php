...
if ( ! class_exists( 'WC_SC_Background_Upgrade' ) ) {
	...
	class WC_SC_Background_Upgrade {
		...
		/**
		 * Holds the current database upgrade version.
		 *
		 * This static variable is used to store the version of the database update that is currently being processed or executed.
		 *
		 * @var int|string|null The current database upgrade version, or null if no upgrade is in progress.
		 */
		public static $current_update_version = '9.22.0';
		...
		/**
		 * Retrieves the current database upgrade version.
		 *
		 * This method returns the value of the static property `$current_update_version`,
		 * which holds the version of the database update that is currently being processed or executed.
		 *
		 * @return int|string|null The current database upgrade version, or null if no upgrade is in progress.
		 */
		public function get_current_update_version() {
			return self::$current_update_version;
		}
		...
		public function get_updates() {
			...
			$updates[] = array(
				'version'         => '9.22.0', // Minimum plugin version to do the action.
				'get_row_handler' => array( __CLASS__, 'get_wc_negative_order_stats_status' ), // get data.
				'cron_handler'    => 'wcsc_fix_order_stats_wc_smart_coupons', // define cron handler which should be in this class.
			);
		...
		private function process( $version = '', $cron_handler = '', $row_handler = '' ) {
			...
			if ( ! empty( $rows ) && is_array( $rows ) ) {
				...
				if ( in_array( $this->get_status( $version ), array( 'failed', 'pending' ), true ) ) {
					if ( function_exists( 'as_enqueue_async_action' ) ) {
						$this->set_status( $version, 'processing' );
						as_enqueue_async_action( $cron_handler, array( 'version' => $version ), $this->group );
					}
				}
			} else {
		...
		/**
		 * Method to initialize WC_SC_Order_Stats_Negative_Net_Total_Fix.
		 */
		public function wcsc_fix_order_stats_wc_smart_coupons() {
			...
			if ( ! empty( $rows ) ) {
				...
				if ( class_exists( 'WC_SC_Order_Stats_Negative_Net_Total_Fix' ) ) {
					$wc_sc_order_stats_negative_net_total_fix = is_callable( array( 'WC_SC_Order_Stats_Negative_Net_Total_Fix', 'get_instance' ) ) ? WC_SC_Order_Stats_Negative_Net_Total_Fix::get_instance() : null;

					if ( $wc_sc_order_stats_negative_net_total_fix instanceof WC_SC_Order_Stats_Negative_Net_Total_Fix && is_callable( array( $wc_sc_order_stats_negative_net_total_fix, 'init' ) ) ) {
						$wc_sc_order_stats_negative_net_total_fix->init();
					}
				}
			}
		}
		...
		/**
		 * Method to fetch fix order stats status.
		 */
		public function get_wc_negative_order_stats_status() {
			$rows = get_transient( 'sc_get_wc_negative_order_stats_status' );
			return $rows;
		}

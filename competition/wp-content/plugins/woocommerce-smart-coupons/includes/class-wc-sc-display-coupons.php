		...
		public function frontend_styles_and_scripts( $args = array() ) {
			...
			if ( apply_filters( 'wc_sc_apply_coupon_via_ajax', true, array( 'source' => $this ) ) ) {
				...
				if ( $has_available_coupons_block_cart && $has_available_coupons_block_checkout ) { // Code to handle apply coupon via blocks.
					...
					$js                = 'let applyCouponAjaxRequest = null;';
			...
			$js .= "var show_hide_coupon_list = function() {
						...
						// Filter to enable or disable available coupon slide animation.
						$js .= apply_filters( 'wc_sc_enable_coupon_slide_animation', true ) ? "jQuery('div#coupons_list').slideDown(800);" : "jQuery('div#coupons_list').show();";
						...
						jQuery(document.body).on('updated_checkout', function( e, data ){
							try {
								if ( data.fragments.wc_sc_available_coupons ) {
									...
									jQuery( document.body ).off('click', '.apply_coupons_credits'); // Remove any existing event
									jQuery( document.body ).on('click', '.apply_coupons_credits', wc_sc_apply_coupon_js);
								}
							} catch(e) {}
							show_hide_coupon_list();

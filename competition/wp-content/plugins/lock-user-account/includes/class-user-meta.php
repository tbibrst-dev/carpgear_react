...
class Baba_User_Meta{
    ...
    public function output_column( $output, $column_name, $user_id ){
        ...
        $locked = get_user_meta( $user_id, sanitize_key( 'lock_account' ), true );
        return ( '1' === $locked ) ? __( 'Locked', 'babatechs' ) : __( 'Not Locked', 'babatechs' );
    }
        ...
        if ( isset( $_GET['_wpnonce'] ) && ! empty( $_GET['_wpnonce'] ) && strpos( wp_get_referer(), '/wp-admin/users.php' ) === 0 ){
            ...
            if( 'lock' === $action ){
                ...
                foreach( $userids as $userid ){
                    ...
                    update_user_meta( (int)$userid, sanitize_key( 'lock_account' ), 'yes' );
                }
            }
            ...
            elseif( 'unlock' === $action ){
                foreach( $userids as $userid ){
                    update_user_meta( (int)$userid, sanitize_key( 'lock_account' ), '' );
                }
            }
        }
    }

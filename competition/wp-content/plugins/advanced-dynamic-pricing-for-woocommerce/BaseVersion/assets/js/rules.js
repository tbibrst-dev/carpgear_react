...
jQuery(document).ready(function ($) {
  ...
  wpc_postboxes._on_expand = function (new_rule, data) {
    ...
    new_rule.find('.rule-date-from-to input[name="rule[additional][date_from]"]').datetimepicker("option", "disabled", false).css("background-color", "white");
    new_rule.find('.rule-date-from-to input[name="rule[additional][date_to]"]').datetimepicker("option", "disabled", false).css("background-color", "white");
  ...
  wpc_postboxes._on_close = function (new_rule, data) {
    ...
    new_rule.find('.rule-date-from-to input[name="rule[additional][date_from]"]').datetimepicker("option", "disabled", true).css("background-color", "#f0f0f1");
    new_rule.find('.rule-date-from-to input[name="rule[additional][date_to]"]').datetimepicker("option", "disabled", true).css("background-color", "#f0f0f1");
  };
  ...
  function preAddEventHandlersToRule(new_rule) {
    ...
    jqDateFrom.removeClass('hasDatepicker').datetimepicker({
      dateFormat: "yy-mm-dd",
      beforeShow: function () {
        let maxDate = new Date(jqDateTo.val());

        jqDateFrom.datetimepicker("option", "maxDate", maxDate);
      },
      disabled: true,
    }).css("background-color", "#f0f0f1");

    jqDateTo.removeClass('hasDatepicker').datetimepicker({
      dateFormat: "yy-mm-dd",
      beforeShow: function () {
        let minDate = new Date(jqDateFrom.val());
        minDate.setDate(minDate.getDate());

        jqDateTo.datetimepicker("option", "minDate", minDate);
      },
      disabled: true,
    }).css("background-color", "#f0f0f1");
  ...
  function update_condition_fields($el, data) {
    ...
    $container.find('[data-field="date"]').removeClass('hasDatepicker').datetimepicker({ dateFormat: "yy-mm-dd" });

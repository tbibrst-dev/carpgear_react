...
if (isset($_GET['download_csv'])) {
    ...
    INNER JOIN {$wpdb->prefix}user_quest ON {$wpdb->prefix}user_quest.order_id = {$wpdb->prefix}competition_tickets.order_id 
    INNER JOIN {$wpdb->prefix}global_questions ON {$wpdb->prefix}global_questions.id = {$wpdb->prefix}user_quest.question_id 
    WHERE competition_id = %d AND {$wpdb->prefix}user_quest.type = 'main' ORDER BY {$wpdb->prefix}competition_tickets.order_id ASC";
...
if (isset($_GET['reward']) && !empty($_GET['reward'])) {
    ...
    INNER JOIN {$wpdb->prefix}comp_reward on {$wpdb->prefix}comp_reward.competition_id = {$wpdb->prefix}competition_tickets.competition_id 
    INNER JOIN {$wpdb->prefix}users on {$wpdb->prefix}users.ID = {$wpdb->prefix}competition_tickets.user_id 
    WHERE {$wpdb->prefix}competition_tickets.reward_milestone <= $milestone_percentage and {$wpdb->prefix}competition_tickets.user_id > 0 and {$wpdb->prefix}competition_tickets.is_purchased = 1 and
    {$wpdb->prefix}competition_tickets.competition_id = %d and  {$wpdb->prefix}comp_reward.id = %d";



    $query = $wpdb->prepare($query, [$_REQUEST['id'], $_GET['reward']]);
    ...
    if (!empty($limit)) {
        ...
        if (!empty($comp_search)) {
            ...
            $total = $wpdb->get_var("SELECT COUNT(*) AS total_records
            FROM ( " . $query . "  LIMIT $limit ) AS limited_records");
    ...
    $num_of_pages = ceil($total / $page_limit);
    ...
    $query .= " ORDER BY {$wpdb->prefix}competition_tickets.user_id  ASC LIMIT $offset, $page_limit";

    $records = $wpdb->get_results($query, ARRAY_A);
}


$reward_winners = $wpdb->get_results("select ticket_id from {$wpdb->prefix}comp_reward_winner where reward_id = " . $_REQUEST['reward'] . " and competition_id = " . $_REQUEST['id'], ARRAY_A);

$winners = array();
foreach ($reward_winners as $reward_winner) {
    $winners[] = $reward_winner['ticket_id'];
}
...
<div id="competitions-plugin-container" class="competition_entries">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <form action="" method="get">
                        <input type="hidden" name="page" value="reward_prizes_entrants" />
                        <input type="hidden" name="id" value="<?php echo $record; ?>" />
                        <input type="hidden" name="reward" value="<?php echo $_GET['reward']; ?>" />
    ...
    <div class="d-flex align-items-center sub_header">
        <a class="btn btn-secondary btn-accent create_btn" type="button" id="reward_entrant_back_btn"
            href="<?php echo admin_url('admin.php?page=reward_prizes&id=' . $record); ?>">
            <span class="order-higher-indicator" aria-hidden="true">
                <?php
                $image_path = '../_inc/img/Vector.png';
        ...
        <h3 class="header-text"><?php echo $comp_info['title']; ?> - Reward Prizes</h3>
    </div>
            ...
            if (!empty($records)) {
                foreach ($records as $record) {
                    ...
                    <tr>
                        ...
                        <td class="normal-text normal-weight"><?php echo $record['user_email']; ?></td>
                        <td class="normal-text normal-weight">
                            <a class="link_text"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer border-0">
                ...
                <a href="#" class="btn make_reward_winner">Award Prize</a>
                ...
                <input type="hidden" id="reward_id" name="reward_id" value="<?php echo $_REQUEST['reward']; ?>" />
            </div>
        </div>
    </div>
</div>

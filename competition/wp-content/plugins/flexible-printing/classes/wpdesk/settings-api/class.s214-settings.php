...
class WPDesk_S214_Settings_1_4 {
	...
	public function render_settings_page() {
		$active_tab = isset( $_GET['tab'] ) && array_key_exists( $_GET['tab'], $this->get_settings_tabs() ) ? $this->sanitize_text_field( $_GET['tab'] ) : $this->default_tab;
		...
		$section = isset( $_GET['section'] ) && ! empty( $registered_sections ) && array_key_exists( $_GET['section'], $registered_sections ) ? $this->sanitize_text_field( $_GET['section'] ) : $key;
	...
	public function settings_sanitize( $input = array() ) {
		...
		if( $doing_section ) {
			...
			$tab     = isset( $referrer['tab'] ) ? sanitize_text_field( $referrer['tab'] ) : $this->default_tab;
			$section = isset( $referrer['section'] ) ? sanitize_text_field( $referrer['section'] ) : 'main';
	...
	public function sysinfo_callback( $args ) {
		...
		if( ! isset( ${$this->func . '_options'}[$args['tab']] ) || ( isset( ${$this->func . '_options'}[$args['tab']] ) && isset( $_GET['tab'] ) && $_GET['tab'] == ${$this->func . '_options'}[$args['tab']] ) ) {
			$html  = '<textarea readonly="readonly" onclick="this.focus(); this.select()" id="system-info-textarea" name="' . $this->func . '-system-info" title="' . __( 'To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).', 'wpdesk-plugin' ) . '">' . esc_textarea( $this->sysinfo->get_system_info() ) . '</textarea>';
			$html .= '<p class="submit">';

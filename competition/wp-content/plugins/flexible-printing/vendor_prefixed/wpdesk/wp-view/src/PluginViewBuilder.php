...
namespace FPrintingVendor\WPDesk\View;

use FPrintingVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use FPrintingVendor\WPDesk\View\Resolver\ChainResolver;
use FPrintingVendor\WPDesk\View\Resolver\DirResolver;
use FPrintingVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \FPrintingVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \FPrintingVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \FPrintingVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \FPrintingVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**

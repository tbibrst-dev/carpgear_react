...
use FPrintingVendor\WPDesk\License\PluginRegistratorInterface;
...
class PluginRegistrator implements \FPrintingVendor\WPDesk\License\PluginRegistratorInterface
{
    ...
    {
        return (new \FPrintingVendor\WPDesk\License\LicenseServer\PluginLicense($this->plugin_info))->is_active();
    }
    public function initialize_license_manager()
    {
        (new \FPrintingVendor\WPDesk\License\LicenseServer\PluginUpgrade($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new \FPrintingVendor\WPDesk\License\LicenseServer\PluginExternalBlocking($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new \FPrintingVendor\WPDesk\License\LicenseServer\PluginViewVersionInfo($this->plugin_info, $this->server))->hooks();
    }
}

...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'flexible-printing'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'flexible-printing'), 'product_status' => \__('Subscription Status', 'flexible-printing'), 'product_license' => \__('Subscription Data', 'flexible-printing')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'flexible-printing');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'flexible-printing');
        }
        return $status;
    }

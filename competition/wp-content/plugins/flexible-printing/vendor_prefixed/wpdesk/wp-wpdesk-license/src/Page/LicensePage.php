...
namespace FPrintingVendor\WPDesk\License\Page;

use FPrintingVendor\WPDesk\PluginBuilder\Plugin\Hookable;
use FPrintingVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \FPrintingVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FPrintingVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    {
        \add_submenu_page('wpdesk-helper', \__('Subscriptions', 'flexible-printing'), \__('Subscriptions', 'flexible-printing'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('FPrintingVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FPrintingVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**

...
use FPrintingVendor\Monolog\Registry;
use FPrintingVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use FPrintingVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FPrintingVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FPrintingVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    {
        if (\FPrintingVendor\Monolog\Registry::hasLogger($name)) {
            \FPrintingVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FPrintingVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FPrintingVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FPrintingVendor\Monolog\Logger($name);
        }
        if (\FPrintingVendor\Monolog\Registry::hasLogger($name)) {
            return \FPrintingVendor\Monolog\Registry::getInstance($name);
        }
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FPrintingVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FPrintingVendor\Monolog\Logger $logger)
    {
        if (!\defined('FPrintingVendor\\WC_LOG_THRESHOLD')) {
            \define('FPrintingVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FPrintingVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FPrintingVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FPrintingVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FPrintingVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FPrintingVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FPrintingVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FPrintingVendor\Monolog\Registry::hasLogger($name);
    }
}

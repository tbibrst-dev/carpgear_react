 ...
 */
class BrowserConsoleHandler extends \FPrintingVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \FPrintingVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\FPrintingVendor\Monolog\Logger::DEBUG => 'debug', \FPrintingVendor\Monolog\Logger::INFO => 'info', \FPrintingVendor\Monolog\Logger::NOTICE => 'info', \FPrintingVendor\Monolog\Logger::WARNING => 'warn', \FPrintingVendor\Monolog\Logger::ERROR => 'error', \FPrintingVendor\Monolog\Logger::CRITICAL => 'error', \FPrintingVendor\Monolog\Logger::ALERT => 'error', \FPrintingVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FPrintingVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FPrintingVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FPrintingVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**

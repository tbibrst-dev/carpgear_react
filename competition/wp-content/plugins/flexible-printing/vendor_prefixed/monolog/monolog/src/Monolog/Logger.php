 ...
 */
namespace FPrintingVendor\Monolog;
 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \FPrintingVendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\FPrintingVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \FPrintingVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \FPrintingVendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \FPrintingVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    public function reset() : void
    {
        ...
        foreach ($this->processors as $processor) {
            if ($processor instanceof \FPrintingVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }

...
use FPrintingVendor\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \FPrintingVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \FPrintingVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \FPrintingVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \FPrintingVendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \FPrintingVendor\Monolog\Formatter\FormatterInterface
    {
        return new \FPrintingVendor\Monolog\Formatter\LogglyFormatter();
    }
}

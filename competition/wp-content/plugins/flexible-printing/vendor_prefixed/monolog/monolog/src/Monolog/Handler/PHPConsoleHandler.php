...
use FPrintingVendor\Monolog\Formatter\LineFormatter;
use FPrintingVendor\Monolog\Formatter\FormatterInterface;
use FPrintingVendor\Monolog\Logger;
use FPrintingVendor\Monolog\Utils;
use FPrintingVendor\PhpConsole\Connector;
use FPrintingVendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use FPrintingVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FPrintingVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\FPrintingVendor\PhpConsole\Connector $connector = null, $level = \FPrintingVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('FPrintingVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\FPrintingVendor\PhpConsole\Connector $connector = null) : \FPrintingVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FPrintingVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FPrintingVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FPrintingVendor\PhpConsole\Helper::isRegistered()) {
            \FPrintingVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FPrintingVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \FPrintingVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \FPrintingVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FPrintingVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FPrintingVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}

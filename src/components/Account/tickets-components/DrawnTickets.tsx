...
type PropsType = {
  ticket: CompetitionType;
};

const DrawnTicket: React.FC<PropsType> = ({ ticket }) => {
  const [displayCount, setDisplayCount] = useState(0);
  const [isShowingMore, setIsShowingMore] = useState(false);

  const drawingToday = isDrawToday(ticket.draw_date);

  const drawingTomorrow = isDrawTomorrow(ticket.draw_date);

  function initialDisplayCount() {
    const numberOfFiveLengthTickets = ticket?.tickets?.filter(
  ...
  useEffect(() => {
    if (isShowingMore) {
      setDisplayCount(ticket?.tickets?.length!);
    } else {
      initialDisplayCount();
    }
  }, [isShowingMore]);
  ...
  return (
    ...
    <div className="upcoming-ticket-section-one">
        ...
        {isShowingMore && (
          <div className="ticket-for-mob">
            ...
            <div className="upcoming-ticket-section-right-ticket-number">
              {ticket.tickets
                ?.map(Number) // Convert all items to numbers
      ...
      <button
        className={`upcoming-tickets-toggle opacity-25 ${ticket.tickets?.length! <= displayCount && "opacity-25"
          }`}
        onClick={() => setIsShowingMore(!isShowingMore)}
        style={{
          opacity: `${ticket?.tickets?.length! < displayCount && 0.5}`,
        }}
        disabled={ticket.tickets?.length! < displayCount}
      >
        {!isShowingMore ? (
          <svg
            width={16}

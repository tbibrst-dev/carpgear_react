...
const ConfirmationPage = () => {
  ...
  useEffect(() => {   
    emptyORClearCart();  
    removeAllCookiesExcept('cggcookie');
  ...
  const removeAllCookiesExcept = (exceptCookie: string) => {
    // Get all cookies
    const allCookies = Cookies.get();
     console.log('Before removal:', Cookies.get());

    // Iterate over all cookies
    for (const cookieName in allCookies) {
      // Remove cookie if it's not the one to keep
      if (cookieName !== exceptCookie) {
        Cookies.remove(cookieName);
      }
    }
    console.log('After removal:', Cookies.get());
  };
  ...
  async function emptyORClearCart() {
    const keys = localStorage.getItem(UPDATE_CART_KEY) as string;
    const parsedKeys = keys ? JSON.parse(keys) : {};
